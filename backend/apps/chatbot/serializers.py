...
class TestKeySerializer(serializers.Serializer):
    ...
    provider = serializers.ChoiceField(
        choices=Chatbot.PROVIDER_CHOICES,
        required=True
    )
...
class ChatbotSerializer(serializers.ModelSerializer):
    ...
    documents_connected = serializers.PrimaryKeyRelatedField(
        many=True,
        read_only=True
    )
    ...
    class Meta:
        model = Chatbot
        fields = [
            "id", "name", "tone", "system_instructions",
            "llm_provider", "llm_model", "llm_api_key", "llm_system_prompt", "llm_is_active",
            "documents_connected", "created_at", "updated_at"
        ]
...
class ChatbotUpdateSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if 'llm_api_key' in validated_data:
            ...
            if api_key:
                instance.llm_api_key = api_key  # Uses the property setter for encryption
...
class ChatbotConfigSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            "id", "name", "tone", "system_instructions",
            "llm_provider", "llm_model", "llm_api_key", "llm_system_prompt", "llm_is_active",
            "documents_connected_ids", "documents_available", "created_at", "updated_at"
        ]
        read_only_fields = ["id", "created_at", "updated_at", "documents_connected_ids", "documents_available"]
