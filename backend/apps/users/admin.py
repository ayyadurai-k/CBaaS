...
class CustomUserChangeForm(UserChangeForm):
    ...
    class Meta:
        model = User
        fields = ('email', 'name', 'role', 'organization', 'phone_number', 'profile_picture', 'is_active', 'is_staff')
...
class UserAdmin(BaseUserAdmin):
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('email', 'name', 'password1', 'password2', 'role', 'organization', 'phone_number', 'is_active', 'is_staff'),
        }),
    )
