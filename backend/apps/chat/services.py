...
def _retrieval(
    org_id, query: str, top_k: int, filters: Dict | None = None
...
def _build_messages(
    user_messages: List[Dict], system_prompt: str, context_blocks: List[str]
) -> List[Dict]:
    context = "\n\n---\n\n".join(context_blocks)
...
def chat_completion(*, org, payload: Dict, model_override: str | None = None) -> Dict:
    ...
    rows, context_blocks = _retrieval(
        org.id, _join_user_text(payload["messages"]), top_k, payload.get("filters")
    )
...
def chat_stream(
    *, org, payload: Dict, model_override: str | None = None
